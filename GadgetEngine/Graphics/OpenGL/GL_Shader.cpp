...
GL_Shader::GL_Shader(const std::string& vertPath_, const std::string& fragPath_) : Shader(), shader(0){
	...
	if(vertCodeStr.empty()){
		Debug::ThrowFatalError(SID("RENDER"), "Could not load vertex shader code from [" + vertPath_ + "]!", __FILE__, __LINE__);
	}

	std::string fragCodeStr = FileSystem::ReadFileToString(fragPath_);
	GADGET_BASIC_ASSERT(!fragCodeStr.empty());
	if(fragCodeStr.empty()){
		Debug::ThrowFatalError(SID("RENDER"), "Could not load fragment shader code from [" + fragPath_ + "]!", __FILE__, __LINE__);
	...
	if(vertShader == 0){
		Debug::ThrowFatalError(SID("RENDER"), "Can't create a new vertex shader!", __FILE__, __LINE__);
	}

	GLuint fragShader = glCreateShader(GL_FRAGMENT_SHADER);
	GADGET_BASIC_ASSERT(fragShader != 0);
	if(fragShader == 0){
		Debug::ThrowFatalError(SID("RENDER"), "Can't create a new shader!", __FILE__, __LINE__);
	}
	...
	if(status == GL_FALSE){
		Debug::ThrowFatalError(SID("RENDER"), "Could not compile Vertex Shader! GL Error: " + GetShaderLog(vertShader), __FILE__, __LINE__);
	}
	...
	GADGET_BASIC_ASSERT(status != GL_FALSE);
	if(status == GL_FALSE){
		Debug::ThrowFatalError(SID("RENDER"), "Could not compile Fragment Shader! GL Error: " + GetShaderLog(fragShader), __FILE__, __LINE__);
	}
	...
	GADGET_BASIC_ASSERT(status != GL_FALSE);
	if(status == GL_FALSE){
		Debug::ThrowFatalError(SID("RENDER"), "Could not link Shader! GL Error: " + GetShaderLog(shader), __FILE__, __LINE__);
	}
