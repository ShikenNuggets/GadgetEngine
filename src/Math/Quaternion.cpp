...
Quaternion Quaternion::Rotate(Angle angle_, const Vector3& axis_){
	...
	return Quaternion(
		Math::CosR(angle_.ToRadiansValue() / 2.0f),
		axis_.x * Math::SinR(angle_.ToRadiansValue() / 2.0f),
		axis_.y * Math::SinR(angle_.ToRadiansValue() / 2.0f),
		axis_.z * Math::SinR(angle_.ToRadiansValue() / 2.0f)
	);
}
...
Vector3 Quaternion::GetRotationAxis(const Quaternion& q_){
	...
	return v / Math::SinR((GetRotationAngle(q_) / 2.0f).ToRadiansValue());
}
...
Quaternion Quaternion::Slerp(const Quaternion& q1_, const Quaternion& q2_, float t_){
	...
	const float sinTheta = Math::Sin(theta.Get());
	...
	//Final Result
	const Quaternion qA = q1 * Math::Sin((1.0f - t_) * theta.Get()) / sinTheta;
	const Quaternion qB = q2 * Math::Sin(t_ * theta.Get()) / sinTheta;
	return qA + qB;
}
