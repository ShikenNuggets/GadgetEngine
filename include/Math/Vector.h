...
namespace Gadget{
	...
	struct Vector2{
		...
		inline constexpr bool IsNear(const Vector2& b_){ return Math::IsNear(x, b_.x) && Math::IsNear(y, b_.y); }
	...
	struct Vector3{
		...
		inline constexpr bool IsNear(const Vector3& b_){ return Math::IsNear(x, b_.x) && Math::IsNear(y, b_.y) && Math::IsNear(z, b_.z); }
	...
	struct Vector4{
		...
		inline constexpr bool IsNear(const Vector4& b_){ return Math::IsNear(x, b_.x) && Math::IsNear(y, b_.y) && Math::IsNear(z, b_.z) && Math::IsNear(w, b_.w); }
